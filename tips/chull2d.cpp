...
void convex_hull(std::vector<Eigen::Vector3d>& p, std::vector<Eigen::Vector3d>& chull)
{
	...
	for (int i = n - 2, t = k+1; i >= 0; --i) {
		while (k >= t && cross(chull[k-2], chull[k-1], p[i]) <= 0) {
                        k--;
                }
                chull[k] = p[i];
                ++k;
...
void save_to_svg (const std::vector<Eigen::Vector3d>& p, const std::vector<Eigen::Vector3d>& chull, const std::string& filename ) {
	...
	fout<<"<polygon points=\"";
	for( int i = 0 ; i < static_cast<int>(chull.size())-1; ++i ) {
